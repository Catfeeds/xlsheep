...
class VipController extends BaseController {
    ...
    public function index() {
        ...
        $time = time();
        $map['rtime'] = ['gt',$time];
        $dfrgk = $morder->where($map)->sum('totalprice');
        $data['dfrgk'] = $dfrgk > 0 ? sprintf('%.2f',$dfrgk) : 0;
        //计算未读消息
        $Model = new Model();
        ...
        $dmap['vipid'] = $vipid;
        if (M("deliveryman")->where($dmap)->find()) {
            $this->assign("isdelivery", 1);
        }
        $this->assign('vip', $vip);
        //vip_apply_status

        $this->assign('dfrgk', $dfrgk);
    ...
    public function txOrder() {
        ...
        $m = M('vip');
        $vip = $m->where('id=' . $vipid)->find();
        $vip['istxcard'] = $vip['txcard'] ? 1 : 0;
        $vip['isopenid'] = $vip['openid'] ? 1 : 0;
        ...
        if (IS_POST) {
            ...
            if ($post['type'] == 'Weui')
                !$vip['openid'] && $this->error('请授权登录微信公众号农牧源');
            ...
            if (FALSE !== $rvip) {
                ...
                if ($post['txtype'] == '1') {
                    $cardtx['sno'] = 'WxPay' . time();
                    $cardtx['openid'] = $vip['openid'];
                }
...
//        $map['type'] = 2; //充值卡充值
    ...
    public function upgrade() {
        ...
        if (self::$WAP['vipset']['first_y'])
            $vip_rule[] = array(1, self::$WAP['vipset']['first_y']);

        if (self::$WAP['vipset']['three_y'])
            $vip_rule[] = array(3, self::$WAP['vipset']['three_y']);

        if (self::$WAP['vipset']['six_y'])
            $vip_rule[] = array(6, self::$WAP['vipset']['six_y']);

        if (self::$WAP['vipset']['twe_y'])
            $vip_rule[] = array(12, self::$WAP['vipset']['twe_y']);
            ...
            if ($paytype == 'money') {
                ...
                if (md5($password) != $pay_password) {
                    ...
                    if (FALSE !== $re) {
                        ...
                        $upgradeData["vip_apply_money"] = $money;
                        ...
                        if (FALSE !== $re) {
                            ...
                            $this->success('已发送合伙人申请！请您等待管理员审核');
                        } else {
                ...
                $upgradeData['vip_apply_type'] = $type;
                $upgradeData["vip_apply_money"] = $money;
                M('vip')->where('id=' . $vipid)->save($upgradeData);
                $_SESSION['wxpaysid'] = 0;
                $_SESSION['wxpayopenid'] = $_SESSION['WAP']['vip']['openid']; //追入会员openid
    ...
    public function bindAdmin() {
        if (IS_POST) {
            ...
            if (strstr($code, "m_")) {
                $m = M("user");
                $codeM = $m->where(array("code" => $code))->find();
                if (!$codeM) {
                    $this->error('该邀请码不存在！');
                }
                if ($codeM["vipid"] > 0) {
                ...
                if ($m->where(array("code" => $code))->save(array("vipid" => $vipid)) !== false) {
                    $this->success('绑定成功！');
                } else {
                    $this->error('绑定失败！');
                }
            } else if (strstr($code, "d_")) {
                $mobile = I("mobile");
                if (!$mobile) {
                    $this->error('电话不能为空！');
                }
                ...
                $m = M("deliveryman");
                $delivery = $m->where(array("code" => $code, "mobile" => $mobile))->find();
                if (!$delivery) {
                    $this->error('该邀请码不存在！');
                }
                if ($delivery["vipid"] > 0) {
                ...
                if ($m->where(array("code" => $code))->save(array("vipid" => $vipid, "uptime" => time())) !== false) {
                    $this->success('绑定成功！');
                } else {
                    $this->error('绑定失败！');
                }
