...
class BaseController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        if ($_SESSION['SET']['wxdebug']) {
            ...
            //$_SESSION['sqopenid'] = 'oLEStv1Y91rvRUKeAIzINFXjbMRc';
            //$_SESSION['userId'] = 1;
            $_SESSION['sqopenid'] = 'oLEStv8m7PhPdzAjUBPkInoxNbgw';
            $_SESSION['userId'] = 85;
        ...
        if(!IS_AJAX) {
        	...
        	if (strtolower(CONTROLLER_NAME) == 'buy' && strtolower(ACTION_NAME) == 'details') {
        		$commonShare = 0;
        	} else {
        		$commonShare = 1;
        	}
        	$this->assign('commonShare', $commonShare);
        	if (strtolower(CONTROLLER_NAME) == 'vip') {
        		$commonShareUrl = "http://{$_SERVER['HTTP_HOST']}/App/Shop/index/ppid/{$_SESSION['WAP']['vipid']}/";
        	} else {
        		$commonShareUrl = get_url();
        	}       			
        	$this->assign('commonShareUrl', $commonShareUrl);
        }
    }
...
//	}
    ...
    function vipInfoset(){
        ...
        if($vipinfo['vip_expiration_time']<time()){
            $result = M('vip') -> where('id = '.$vipinfo['id']) -> setField('groupid',1);
        }
