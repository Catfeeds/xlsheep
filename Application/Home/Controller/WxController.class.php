...
//        }
    ...
    public function checkKeyword($key)
    {
        ...
        if ($key == "员工二维码") {
            ...
            imagecopyresampled($background, $qrcode, $b_width * 0.24, $b_height * 0.5, 0, 0, $q_width * 1, $q_height * 1, $q_width, $q_height);
            ...
            if (file_exists($filepath)) {
            	if (class_exists('\CURLFile')) {
            		$data= array('fieldname' => new \CURLFile(realpath($filepath)));
            	} else {
            		$data= array('fieldname' => '@'.realpath($filepath));
            	}
        ...
        if ($key == "推广二维码") {
            ...
            if (!$vip) {
                ...
                $msg = "您还未成为" . self::$_shop['fxname'] . "，请先申请成为" . self::$_shop['fxname'] . "！";
                self::$_wx->text($msg)->reply();
                exit();
            }
            ...
            $h_width = imagesx($headimg);
            $h_height = imagesy($headimg);
            imagecopyresampled($background, $qrcode, $b_width * 0.30, $b_height * 0.54, 0, 0, 222, 220, $q_width, $q_height);
            imagecopyresampled($background, $headimg, $b_width * 0.10, 36, 0, 0, 120, 120, $h_width, $h_height);
            ...
            // Combine All And Text, Then store in local
            imagettftext($background, 18, 0, 255, 86, $fontcolor, $fonttype, $vip['nickname']);
        ...
        switch ($event) {
            ...
            case 'subscribe':
                ...
                if ($isold) {
                    ...
                    if ($pid > 0) {
                    	$pvip = self::$_fx -> where('id = '.$pid) -> find();
                    	if(!empty($pvip)){
                    		$rfxs = self::$_fx->where('id=' . $pid)->setInc('total_xxsub', 1);
                    	}
                    }
                    ...
                    if ($user) {
                        ...
                        $dlog['to'] = 0;
                        $dlog['toname'] = $old ? $old['nickname'] : self::$_shop['name'];
                        $dlog['issub'] = 1;
                        $dlog['ctime'] = time();
                        $rdlog = self::$_fxlog->add($dlog);
                        ...
                        if (self::$_shop['isfx'] && $old) {
                            ...
                            if ($tj_money > 0) {
                                ...
                                if (FALSE !== $rold) {
                                    ...
                                    $flow['money'] = $tj_money;
                                    $flow['paytype'] = '';
                                    $flow['balance'] = M('Vip')->where('id='.$old['id'])->getField('money');
                                    $flow['type'] = 11;
                                    $flow['oid'] = '';
                                    $flow['ctime'] = time();
                ...
                if ($old) {
                    ...
                    if ($old['pid']) {
                        ...
                        if ($fxs) {
                            ...
                            $dlog['ctime'] = time();
                            $rdlog = self::$_fxlog->add($dlog);
                            $rfxs = self::$_fx->where('id=' . $old['pid'])->setInc('total_xxunsub', 1);    //下线累计取消关注
                        }
                    } else {
                        $dlog['ppid'] = 0;
                        $dlog['from'] = $old['id'];
...
//            \Util\QRcode::png($url, './QRcode/qrcode/' . $openid . '.png', 'L', 6, 2);
    ...
    function subscribeReturn($msg)
    {
        ...
        if (self::$_set['wxswitch'] == '0' || !$switchs) {
        	...
        	if (file_exists($temp)) {
	            if (class_exists('\CURLFile')) {
	            	$data = array('fieldname' => new \CURLFile(realpath($temp)));
	            } else {
	            	$data = array('fieldname' => '@'.realpath($temp));
	            }
	            $uploadresult = self::$_wx->uploadMedia($data, 'image');
	            $log = serialize($uploadresult);
	            file_put_contents('./Data/test.txt', $log);
	            self::$_wx->image($uploadresult['media_id'])->reply();
        	}
        }
    }
