...
document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
	WeixinJSBridge.invoke('getBrandWCPayRequest', {
		...
		if(res.err_msg == 'get_brand_wcpay_request:ok') {
			//alert('您已成功付款！ ');
			if($$wOpt['type']==0){
				window.location.href='http://<?php echo $_SERVER['HTTP_HOST'];?>/App/Shop/orderList/type/5';
            }else if($$wOpt['type']==1){
            	window.location.href='http://<?php echo $_SERVER['HTTP_HOST'];?>/App/Activity/groupShare/id/'.$wOpt['actid'];
            }else{
            	window.location.href='http://<?php echo $_SERVER['HTTP_HOST'];?>/App/Shop/orderList/type/7';
            }	
		} else {
			console.error(res);
