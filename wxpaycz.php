...
$wOpt = $_GET;
$wOpt['orderMake'] = base64_decode($wOpt['orderMake']);
...
document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
	WeixinJSBridge.invoke('getBrandWCPayRequest', {
		...
		var orderurl = '<?php echo $wOpt['orderMake'];?>';
		if(res.err_msg == 'get_brand_wcpay_request:ok') {
			//alert('您已成功付款！ ');
			if(orderurl){
				window.location.href='http://<?php echo $_SERVER['HTTP_HOST'];?>'+orderurl;
			}else{
				window.location.href='http://<?php echo $_SERVER['HTTP_HOST'];?>/App/Vip/index/';
			}
		} else {
